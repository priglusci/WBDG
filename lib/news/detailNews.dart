...
import 'package:wbdg/news/commentCardNews.dart';
import 'package:wbdg/news/commentNEWS.dart';
...
class newsScreen extends StatefulWidget {
  ...
  newsScreen(this.id,this.image,this.judul,this.body,this.subJudul,this.penulis,
      {Key? key})
      : super(key: key);
  @override
  State<newsScreen> createState() =>
      _newsScreenState(id,image,judul,body,subJudul,penulis);
}
...
class _newsScreenState extends State<newsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    CollectionReference komentarNews = firestore.collection('users');
                         ...
                         child:Column(
                           ...
                           children: [
                             ...
                             StreamBuilder<QuerySnapshot>(
                                 stream: komentarNews.where("_komentarNews", isNotEqualTo: "").snapshots(),
                                 builder: (_,snapshot){
                                   if(snapshot.hasData){
                                     return Column(
                                       children: (snapshot.data! as QuerySnapshot).docs.map((e) =>commentCard(Komentar(
                                         name: e['name'],
                                         comment: e['_komentarNews'],
                                       ),
                                       ),).toList(),
                                     );
                                   }else{
                             ...
                             GestureDetector(
                               onTap: () async{
                                 komentarNews.where("_komentarNews", isEqualTo: "");
                                 await FirebaseFirestore.instance.collection("users")
                                     .doc(uid)
                                     .update({
                                   '_komentarNews':komencontroller.text,
                                 });
                                 print(uid);
                               },
                               child: Container(
